import { userModel } from "../../../models/user.model";
import jwt from "jsonwebtoken";

export default defineEventHandler(async (event) => {
  try {
    let { token } = event.req.headers;
    let id = getRouterParam(event, "id");
    let runtimeConfig = useRuntimeConfig();

    const verifyToken = () => {
      return new Promise((resolve, reject) => {
        jwt.verify(token, runtimeConfig.SECRET_KEY, (err, decoded) => {
          if (err) {
            reject(err);
          } else {
            resolve(decoded);
          }
        });
      });
    };

    let decoded = await verifyToken();
    console.log(id);
    let requestSent = [];
    let requestReceive = [];
    let myFriends = [];
    let otherFriend = [];

    let user = await userModel.findById(decoded.userId);
    myFriends = [...user.friends, id];
    requestReceive = user.requestReceive.filter((item) => item != id);
    await userModel.findByIdAndUpdate(decoded.userId, {
      requestReceive: requestReceive,
      friends: myFriends,
    });

    let userVisit = await userModel.findById(id);
    otherFriend = [...userVisit.friends, decoded.userId];
    requestSent = userVisit.requestSent.filter(
      (item) => item != decoded.userId
    );

    await userModel.findByIdAndUpdate(id, {
      requestSent: requestSent,
      friends: otherFriend,
    });
    return {
      msg: "Add accept successfully",
    };
  } catch (err) {
    console.error(err);
    return { msg: "error" };
  }
});
