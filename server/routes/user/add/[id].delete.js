import { userModel } from "../../../models/user.model";
import jwt from "jsonwebtoken";

export default defineEventHandler(async (event) => {
  try {
    let { token } = event.req.headers;
    let id = getRouterParam(event, "id");
    let runtimeConfig = useRuntimeConfig();

    const verifyToken = () => {
      return new Promise((resolve, reject) => {
        jwt.verify(token, runtimeConfig.SECRET_KEY, (err, decoded) => {
          if (err) {
            reject(err);
          } else {
            resolve(decoded);
          }
        });
      });
    };

    let decoded = await verifyToken();
    let myFriends = [];
    let otherFriend = [];

    let user = await userModel.findById(decoded.userId);
    myFriends = user.friends.filter((item) => item != id);
    await userModel.findByIdAndUpdate(decoded.userId, {
      friends: myFriends,
    });

    let userVisit = await userModel.findById(id);
    otherFriend = userVisit.friends.filter((item) => item != decoded.userId);
    await userModel.findByIdAndUpdate(id, { friends: otherFriend });
    return {
      msg: "add deleted successfully",
    };
  } catch (err) {
    console.error(err);
    return { msg: "error" };
  }
});
